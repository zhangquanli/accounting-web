...
const initialVoucher: Voucher = {
  accountDate: moment(),
  accountingEntries: [{ key: nanoid() }, { key: nanoid() }],
};
...
const VoucherTemplate: FC<Props> = ({ voucherId, onSave, onInvalid }) => {
  ...
  useEffect(() => {
    (async () => {
      if (activeAccountId) {
        ...
        const newSubjectBalances = subjectBalances.map(
          (subjectBalance: any) => {
  ...
  useEffect(() => {
    (async () => {
      if (voucherId) {
        ...
        const newAccountingEntries = accountingEntries.map(
          (accountEntry: any) => {
            const { id, summary, type, amount, labels, subjectBalance } =
              accountEntry;
            const subjectIds = subjectBalance.subject.num
              .split("-")
  ...
  const deleteAccountEntry = (key: string) => {
    const { accountingEntries } = voucher;
    const newAccountEntries = accountingEntries.filter(
      (item) => item.key !== key
    );
    setVoucher({ ...voucher, accountingEntries: newAccountEntries });
  };
  ...
  const moneyAmount = (type: "DEBIT" | "CREDIT") => {
    const amount = voucher.accountingEntries
  ...
  const saveVoucher = async () => {
    ...
    const newAccountingEntries = voucher.accountingEntries
      .filter((item) => {
  ...
  return (
    <div className={styles.container}>
        ...
        <Row key={item.key}>
            ...
            <Select
              ...
              onSelect={(value: any) => {
                if (!value.includes("-")) {
                  ...
                  updateAccountEntry(item.key, [
                    { name: "labels", value: newLabels },
                  ]);
                  message.destroy();
                ...
                if (item.labels && item.labels.length > 0) {
                  const newLabels = item.labels.filter(
                    (mark) => mark.split("-")[0] !== value.split("-")[0]
                  );
            ...
            <Cascader
              ...
              showSearch={{
                filter: (inputValue, options) => {
                  return options.some(
                    (option: any) =>
                      option.name.indexOf(inputValue.toLowerCase()) > -1
                  );
                },
              }}
              onChange={(value: any) => {
              ...
              displayRender={(nodes, selectedOptions: any) => {
                return (selectedOptions || []).map(
                  (option: any, index: number) => {
                    console.log("凭证模板", selectedOptions);
                    if (index === selectedOptions.length - 1) {
                      ...
                      return (
                        <span key={option.id}>{option.subject?.name} / </span>
                      );
                    }
                  }
                );
          ...
          <Col className={`${styles.border} ${styles.center}`} span={2}>
            <InputNumber
              ...
              bordered={false}
              value={
                item.type && item.type === "DEBIT" ? item.amount : undefined
              }
              onChange={(value) => {
                const pairs = [
          ...
          <Col className={`${styles.border} ${styles.center}`} span={2}>
            <InputNumber
              ...
              bordered={false}
              value={
                item.type && item.type === "CREDIT" ? item.amount : undefined
              }
              onChange={(value) => {
                const pairs = [
      ...
      <Row>
        <Col className={`${styles.border} ${styles.center}`} span={12}>
          合计
        </Col>
        <Col className={`${styles.border} ${styles.center}`} span={8}>
          {debitAmount && creditAmount && debitAmount === creditAmount ? (
            <div style={{ color: "green" }}>
              {capitalAmount(Math.abs(debitAmount))}
            </div>
          ) : (
      ...
      <Row style={{ marginTop: "1rem" }}>
        <Col className={styles.center} span={24}>
          {/*新增凭证时，显示保存按钮*/}
          {!voucher.id ? (
            <Button type="link" onClick={saveVoucher}>
              保存
            </Button>
          ) : null}
          {/*查看凭证时，显示冲红按钮*/}
          {voucher.id &&
          !(voucher.invalidVoucher || voucher.originalVoucher) ? (
            <Button
              type="link"
              danger={true}
              onClick={() => invalidVoucher(voucherId)}
            >
              冲红
            </Button>
          ) : null}
